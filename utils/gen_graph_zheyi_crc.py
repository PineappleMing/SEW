...
# pixel_2_label = {-1: 0, 0: 0, 59: 0, 70: 1, 73: 2, 82: 3, 109: 4, 117: 5, 120: 6, 160: 7, 161: 8, 170: 9, 217: 10,
#                  255: 11}

zheyi_crc_root = "/mnt/medical-data/yxt/changai/"
# pos_list = pd.read_csv('/mnt/s3/lhm/zheyi_crc/visual_data_list_p.csv')["train"].to_list()
# neg_list = pd.read_csv('/mnt/s3/lhm/zheyi_crc/visual_data_list_n.csv')["train"].to_list()
dst_root = "/mnt/s3/lhm/zheyi_crc_4/"
def convert_numpy_img_to_superpixel_graph(img, code, prefix, n=1024):
    ...
    # ,mask=mask)
    if not os.path.exists(f"{dst_root}segments/{code}"):
        os.mkdir(f"{dst_root}segments/{code}")
    np.save(f"{dst_root}segments/{code}/{prefix}.npy", segments)
...
def process_img(i):
    ...
    res['code'] = code
    res['nodey'] = y
    if not os.path.exists(f'{dst_root}/graph/{code}'):
        os.mkdir(f'{dst_root}/graph/{code}')
    np.save(f'{dst_root}/graph/{code}/{prefix}.npy', res, allow_pickle=True)


args_mat = glob.glob(dst_root + 'patch/*/*.ori.jpg')
# print(args_mat)
multi_process_exec(process_img, args_mat, 64, desc='processing')
