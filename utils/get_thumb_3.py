...
for i in tqdm.tqdm(slides):
    ...
    w, h = s.dimensions
    img = np.array(s.read_region((w // 2, h // 2), 0, (1024, 1024)).convert('RGB'))
    if img is not None:
        cv2.imwrite('/home/lhm/mnt/medical/lhm/liverWSI/level3/' + name + '.png', img)
        print('saved to' + '/home/lhm/mnt/medical/lhm/liverWSI/level3' + name + '.png')
